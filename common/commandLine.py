...
class CommandLine:
	...
	def __setArgument(self, arg):
		if arg[0]=="-i":
			if 'method' in self.options: self.__paramError("specify one input source only")
			self.options['method'] = 'live'
			self.options['interface'] = arg[1]

		elif arg[0]=="-r":
			if 'method' in self.options: self.__paramError("specify one input source only")
			self.options['method'] = 'file'
			self.options['filename'] = arg[1]

		elif arg[0]=="-w":
			self.options['dumpfile'] = arg[1]
			...
			try:
				...
				self.options['verbosity'] = int(arg[1])

		else:
			self.__paramError("parameter {0} not recognized".format(arg[0])) 

	def __checkMandatoryArgs(self):
		if not 'method' in self.options: self.__paramError("specify input method")
